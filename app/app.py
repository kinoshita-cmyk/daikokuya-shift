...
def detect_short_staff_by_store(shift: MonthlyShift) -> dict[int, set[Store]]:
    ...
    - スキル構成の注意は検証結果側で表示する
    """
    from prototype.staffing import detect_short_staff_by_store as _detect

    return _detect(shift)
...
def push_lock_file_to_github(lock_path: Path, year: int, month: int, action: str) -> tuple[bool, str]:
    """ロック情報をGitHubへ自動保存する。"""
    try:
        from prototype.github_backup import push_lock_to_github
        return push_lock_to_github(lock_path, int(year), int(month), action=action)
    except Exception as e:
        return False, f"{type(e).__name__}: {e}"


def push_finalized_lock_bundle_to_github(
    snapshot_path: Path,
    lock_path: Path,
    year: int,
    month: int,
) -> dict:
    """確定シフト本体とロック情報を、復元に必要なセットとしてGitHubへ保存する。"""
    result = {
        "success": False,
    ...
    try:
        ...
        result["github_enabled"] = bool(is_github_backup_enabled())
        if not result["github_enabled"]:
            result["messages"].append("GitHub自動バックアップが未設定です。")
            return result

        shift_ok, shift_msg = push_shift_to_github(
            snapshot_path, int(year), int(month), kind="finalized",
        )
        result["messages"].append(f"確定版シフト: {'OK' if shift_ok else 'NG'} - {shift_msg}")

        lock_ok, lock_msg = push_lock_to_github(
            lock_path, int(year), int(month), action="lock",
        )
        result["messages"].append(f"ロック情報: {'OK' if lock_ok else 'NG'} - {lock_msg}")

        sync_key = f"{int(year):04d}-{int(month):02d}"
        snapshot_repo_path = f"backups/{sync_key}/{Path(snapshot_path).name}"
        snapshot_exists, snapshot_exists_msg = github_file_exists(snapshot_repo_path)
        result["messages"].append(
            f"復元用シフト本体の確認: "
            f"{'OK' if snapshot_exists else 'NG'} - {snapshot_repo_path} / {snapshot_exists_msg}"
        )
        latest_lock_path = f"locks/{sync_key}.lock"
        lock_exists, lock_exists_msg = github_file_exists(latest_lock_path)
        result["messages"].append(
            f"復元用ロック情報の確認: "
            f"{'OK' if lock_exists else 'NG'} - {latest_lock_path} / {lock_exists_msg}"
        )

        result["success"] = bool(shift_ok and lock_ok and snapshot_exists and lock_exists)
        return result
    except Exception as e:
        result["messages"].append(f"GitHub保存処理で例外: {type(e).__name__}: {e}")
        return result


def load_backup_debug_events(limit: int = 120) -> list[dict]:
    """バックアップ診断ログの直近分を読む。"""
    debug_dir = BACKUP_DIR / "debug"
    if not debug_dir.exists():
        return []
    events: list[dict] = []
    for log_path in sorted(debug_dir.glob("backup_debug_*.jsonl"))[-7:]:
        try:
...
def active_monthly_required_assignment_rules(
    rule_cfg: RuleConfig,
    year: int,
    month: int,
) -> list[dict]:
    """対象年月に有効な、日付指定の月別配置ルールを取り出す。"""
    active_rules = []
    for rule in getattr(rule_cfg, "custom_rules", []):
        if not getattr(rule, "enabled", True):
            continue
        if getattr(rule, "rule_type", "note") != "required_assignment":
            continue
        try:
            target_year = getattr(rule, "target_year", None)
            target_month = getattr(rule, "target_month", None)
            ...
            if target_month is not None and int(target_month) != int(month):
                continue
            day = int(getattr(rule, "day", 0) or 0)
            if day <= 0:
                continue
        except (TypeError, ValueError):
            continue
        active_rules.append({
            ...
            "employee": getattr(rule, "employee", ""),
            "day": day,
            "store": (list(getattr(rule, "stores", []) or [])[:1] or [""])[0],
            "stores": list(getattr(rule, "stores", []) or []),
...
def monthly_rule_display_text(rule: dict) -> str:
    ...
    if rule.get("day"):
        return (
            ...
            f"{int(rule.get('day'))}日 / {store_label}に配置"
        )
    return (
        f"{rule.get('name', '月別ルール')}: "
        f"{rule.get('employee') or 'スタッフ指定なし'} / "
                ...
                with st.expander("📋 生成時の入力データ（クリックで展開）", expanded=(_gen_status != "success")):
                    ...
                    if _isum.get("monthly_store_count_rules"):
                        ...
                        for rule in _isum["monthly_store_count_rules"]:
                            st.write(f"    - {monthly_rule_display_text(rule)}")
                    if _isum.get("required_assignments"):
                        st.write("- 月別の日付指定配置:")
                        for rule in _isum["required_assignments"]:
                            st.write(f"    - {monthly_rule_display_text(rule)}")
        ...
        if st.button(
            ...
            try:
                ...
                with st.spinner(f"シフト案を生成中... (最大{solver_limit_seconds}秒)"):
                    ...
                    use_required_assignments = active_monthly_required_assignment_rules(
                        rule_cfg, _saved_target_year, _saved_target_month,
                    )
                    ...
                    if "required_assignments" in generator_params:
                        generation_kwargs["required_assignments"] = use_required_assignments
        ...
        with st.form("lock_form", clear_on_submit=True):
            ...
            if submit_lock and current_shift is not None:
                ...
                lock_path = lock_mgr.lock(
                    year=_lock_year, month=_lock_month,
                    locked_by=lock_author,
                    snapshot_file=snapshot_path.name,
                    note=lock_note,
                )
                gh_result = push_finalized_lock_bundle_to_github(
                    snapshot_path, lock_path, _lock_year, _lock_month,
                )
                st.session_state["last_lock_backup_result"] = {
                    "year": _lock_year,
                    "month": _lock_month,
                    "snapshot_file": snapshot_path.name,
                    **gh_result,
                }
                if gh_result.get("github_enabled") and not gh_result.get("success"):
                    try:
                        lock_path.unlink(missing_ok=True)
                    except Exception:
                        pass
                    st.error(
                        "❌ ロックは完了していません。GitHubバックアップへの保存に失敗しました。\n\n"
                        "このままロック済みにすると、Streamlit再起動後に消える可能性があるため、"
                        "ローカルのロックを取り消しました。"
                    )
                    with st.expander("保存失敗の詳細", expanded=True):
                        for msg in gh_result.get("messages", []):
                            st.write(f"- {msg}")
                    st.stop()
                if not gh_result.get("github_enabled"):
                    st.warning(
                        "⚠ GitHub自動バックアップが未設定のため、ローカルだけでロックしました。"
                        "Streamlit Cloudでは再起動時に消える可能性があります。"
                    )
                else:
                    st.success("✅ GitHubバックアップにも確定版シフトとロック情報を保存しました。")
                    with st.expander("保存確認の詳細", expanded=False):
                        for msg in gh_result.get("messages", []):
                            st.write(f"- {msg}")
                st.success(f"✅ {_lock_year}年{_lock_month}月をロックしました")
        ...
        if selected_shift_view == "📋 シフト表":
            ...
            if available_ai_providers:
                ...
                if _cv_match:
                    ...
                    _cv_required_assignments = _cv_inputs.get("required_assignments", [])
                else:
                    _cv_work = []
                    _cv_preferred_work = []
...
# ============================================================
    ...
    with setting_tab_exceptions:
        st.info(
            "💡 この画面は「📊 経営者ビュー → ⚙️ 2. 今月の条件を整える」にも"
            "表示されるようになりました。シフト調整中はそちらが便利です。"
        )
        render_monthly_exceptions_panel()
    ...
    with setting_tab1:
        ...
        st.markdown("#### 対象月に適用される主要ルール")
        st.caption(
            "全月共通の基本ルールに、選択した月だけの例外や数値変更を"
            "反映した最終条件です。シフトの生成・検証で実際に参照する"
            "主要なルールを対象月ごとに表示します。"
        )
        ...
        rule_inventory = [
            ...
            _rule_row(
                "スタッフ別", "従業員別の店舗適性",
                "絶対担当・主担当・通常対応可・応援/巡回可・絶対配置不可を生成時の重みとして使う。",
                "一部反映", "未接続", "従業員マスタで管理",
                "従業員マスタ", "一部反映",
                "割合の厳密チェックではなく、現状は配置の好みとして扱います。研修は月別ルールまたは手動調整で扱います。",
            ),
            _rule_row(
                "スタッフ別", "南さんの出勤希望日のみ稼働",
            ...
            _rule_row(
                "スタッフ別", "大塚さんのパート運用",
                "年間基準勤務日数の対象外。最大連勤など必要なチェックのみ適用。",
                "反映中", "一部反映", "検証結果に表示",
                "コード固定", "反映中",
                "2026年5月の10日勤務固定は、過去月の例外としてのみ残しています。",
            ),
            _rule_row(
                "スタッフ別", "山本さん補助ロジック",
    ...
    with setting_tab5:
        ...
        if is_github_backup_enabled():
            ...
            st.markdown("##### ロック・確定版シフトのGitHub診断")
            st.caption(
                "月ごとに、GitHubバックアップ上へロック情報・確定版シフト・下書きが"
                "保存されているか確認できます。Streamlitの再起動後に復元できるかを見るための欄です。"
            )
            diagnostic_months: set[str] = set(data_summary.get("submissions_by_month", {}).keys())
            try:
                for month_dir in BACKUP_DIR.iterdir():
                    if month_dir.is_dir() and re.fullmatch(r"\d{4}-\d{2}", month_dir.name):
                        diagnostic_months.add(month_dir.name)
            except Exception:
                pass
            try:
                for lock_item in ShiftLockManager().list_locks():
                    diagnostic_months.add(f"{int(lock_item.year):04d}-{int(lock_item.month):02d}")
            except Exception:
                pass
            try:
                diagnostic_months.add(f"{int(target_year):04d}-{int(target_month):02d}")
            except Exception:
                pass
            available_diagnostic_months = sorted(diagnostic_months, reverse=True)
            if available_diagnostic_months:
                default_diag_index = (
                    available_diagnostic_months.index("2026-06")
                    if "2026-06" in available_diagnostic_months else 0
                )
                diagnostic_ym = st.selectbox(
                    "診断する月",
                    options=available_diagnostic_months,
                    index=default_diag_index,
                    key="github_backup_diagnostic_month",
                )
                diag_year, diag_month = [int(x) for x in diagnostic_ym.split("-", 1)]
                dcol1, dcol2, dcol3 = st.columns(3)
                with dcol1:
                    if st.button("この月のGitHub保存状況を診断", key="diagnose_github_backup_month"):
                        with st.spinner("GitHubバックアップを確認しています..."):
                            st.session_state["github_backup_diag_result"] = diagnose_github_backup_month(
                                diag_year, diag_month,
                            )
                with dcol2:
                    if st.button("GitHubからこの月を再取得", key="sync_lock_from_github_now"):
                        with st.spinner("GitHubからロック・確定版シフトを取得しています..."):
                            ok, msg = sync_latest_lock_and_snapshot_from_github(
                                diag_year, diag_month, BACKUP_DIR, LOCK_DIR, force=True,
                            )
                        if ok:
                            st.success(f"✅ {msg}")
                        else:
                            st.warning(f"⚠ {msg}")
                with dcol3:
                    if st.button("ローカルのロック確定版をGitHubへ同期", key="push_local_lock_bundle_now"):
                        local_lock_mgr = ShiftLockManager()
                        local_lock_info = local_lock_mgr.get_lock_info(diag_year, diag_month)
                        if local_lock_info is None:
                            st.error("この月はローカル上でロックされていないため、同期できません。")
                        else:
                            local_snapshot_path = (
                                BACKUP_DIR
                                / f"{diag_year:04d}-{diag_month:02d}"
                                / local_lock_info.snapshot_file
                            )
                            local_lock_path = LOCK_DIR / f"{diag_year:04d}-{diag_month:02d}.lock"
                            if not local_snapshot_path.exists() or not local_lock_path.exists():
                                st.error(
                                    "ロック情報または確定版シフト本体がローカルに見つかりません。"
                                    "まずバックアップZIPの復元、またはGitHubからの再取得を確認してください。"
                                )
                            else:
                                with st.spinner("GitHubへ確定版シフトとロック情報を保存しています..."):
                                    bundle_result = push_finalized_lock_bundle_to_github(
                                        local_snapshot_path,
                                        local_lock_path,
                                        diag_year,
                                        diag_month,
                                ...
                                if bundle_result.get("success"):
                                    st.success("✅ GitHubへロック確定版を同期しました。")
                                else:
                                    st.error("❌ GitHubへの同期に失敗しました。")
                                with st.expander("同期結果の詳細", expanded=True):
                                    for msg in bundle_result.get("messages", []):
                                        st.write(f"- {msg}")

                diag_result = st.session_state.get("github_backup_diag_result")
                if diag_result and diag_result.get("ym") == diagnostic_ym:
                    m1, m2, m3, m4 = st.columns(4)
                    with m1:
                        st.metric("最新ロック", "あり" if diag_result.get("lock_latest_exists") else "なし")
                    with m2:
                        st.metric("確定版", f"{diag_result.get('finalized_shift_count', 0)} 件")
                    with m3:
                        st.metric("下書き", f"{diag_result.get('draft_shift_count', 0)} 件")
                    with m4:
                        st.metric("提出", f"{diag_result.get('preference_count', 0)} 件")
                    with st.expander("診断メッセージ", expanded=True):
                        for msg in diag_result.get("messages", []):
                            st.write(f"- {msg}")
                    with st.expander("ファイル名の詳細", expanded=False):
                        if diag_result.get("finalized_files"):
                            st.markdown("**確定版シフト**")
                            for name in diag_result.get("finalized_files", [])[-10:]:
                                st.write(f"- {name}")
                        if diag_result.get("draft_files"):
                            st.markdown("**下書きシフト**")
                            for name in diag_result.get("draft_files", [])[-10:]:
                                st.write(f"- {name}")
                        if diag_result.get("lock_history_files"):
                            st.markdown("**ロック履歴**")
                            for name in diag_result.get("lock_history_files", [])[-10:]:
                                st.write(f"- {name}")

            with st.expander("バックアップ診断ログ（直近）", expanded=False):
                debug_events = load_backup_debug_events()
                if not debug_events:
                    st.caption("まだ診断ログはありません。")
                else:
                    debug_rows = []
